...
Uses Mitsuba's real `path` integrator in an RGB LLVM variant when available.
All numeric outputs are linear radiance; no exposure fitting or tone mapping.
"""
...
def main():
    ...
    preferred = "llvm_ad_rgb" if "llvm_ad_rgb" in variants else (
        "scalar_rgb" if "scalar_rgb" in variants else None
    )
    ...
    render_scene("diffuse_env", diffuse_scene, {
        "variant":preferred,
        "semantic_contract":"Lambertian rho + constant environment radiance",
        "rho":RHO,"environment_radiance":ENV,
    })

    glass_obj = OUT/"thin_glass.obj"
    panel_obj = OUT/"emissive_panel.obj"
    # VPT representational slab indices z=30..34 has front face z=3.5 from +z.
    write_quad_obj(glass_obj,1.2,5.2,0.6,3.4,3.5,True)
    # VPT emissive voxel layer z=19 has camera-facing surface z=2.0.
    ...
    render_scene("thin_emitter", thin_scene, {
        "variant":preferred,
        "semantic_contract":"zero-thickness paired-interface dielectric sheet",
    ...
    manifest = {
        ...
        "variant":preferred,"available_variants":variants,
        "spp":SPP,"seed":SEED,"width":W,"height":H,
